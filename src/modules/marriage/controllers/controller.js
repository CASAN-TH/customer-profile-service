...
exports.getByID = function (req, res, next, id) {
    ...
    Marriage.findById(id, function (err, data) {
        if (err) {
            return res.status(400).send({
                status: 400,
                message: errorHandler.getErrorMessage(err)
...
exports.update = function (req, res) {
    var updMarriage = _.extend(req.data, req.body);
    updMarriage.updated = new Date();
    updMarriage.updateby = req.user;
    updMarriage.save(function (err, data) {
        if (err) {
            return res.status(400).send({
                status: 400,
                message: errorHandler.getErrorMessage(err)
