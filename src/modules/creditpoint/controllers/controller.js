...
exports.getByID = function (req, res, next, id) {
    ...
    Creditpoint.findById(id, function (err, data) {
        if (err) {
            return res.status(400).send({
                status: 400,
                message: errorHandler.getErrorMessage(err)
...
exports.getByUserId = function (req, res, next) {
    Creditpoint.findOne({ u_id: req.user.username }, function (err, data) {
        if (err) {
            return res.status(400).send({
                status: 400,
...
exports.update = function (req, res) {
    var updCreditpoint = _.extend(req.data, req.body);
    updCreditpoint.updated = new Date();
    updCreditpoint.updateby = req.user;
    updCreditpoint.save(function (err, data) {
        if (err) {
            return res.status(400).send({
                status: 400,
                message: errorHandler.getErrorMessage(err)
